// Pixel Arc — Originkit  h
// https://www.originkit.dev/components/pixel-arc?preset=base
...
function resolveCssVar(str, canvasEl) {
    // Resuelve var(--nombre) o var(--nombre, fallback) contra el DOM real
    const m = str.match(/^var\(\s*(--[\w-]+)\s*(?:,\s*(.+))?\)$/)
    if (!m) return str
    const varName = m[1]
    ...
    return inlineFallback ? inlineFallback.trim() : str
}

function parseColor(input, fb, canvasEl) {
    if (!input) return fb
    let str = String(input).trim()
    if (str.startsWith("var(")) {
        str = resolveCssVar(str, canvasEl)
    }
    if (str.charAt(0) === "#") {
        let hex = str.slice(1)
...
function __OriginkitBase_DataPixelArc(props) {
    const {
        style,
    ...
    useEffect(() => {
        ...
        const render = (now) => {
            ...
            const cg = parseColor(v.bg, [0.012, 0.012, 0.031], canvas)
            const cb = parseColor(v.base, [0.118, 0.392, 0.22], canvas)
            const ca = parseColor(v.accent, [0.133, 0.863, 0.314], canvas)
            const chh = parseColor(v.high, [0.918, 1.0, 0.941], canvas)
...
export default function DataPixelArc(props) {
  return <__OriginkitBase_DataPixelArc {...__originkitPresetProps} {...props} />
}